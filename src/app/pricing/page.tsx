...
export default function PricingPage() {
  ...
  return (
    <main className="lamidone-section min-h-screen pt-28 pb-24 px-4">

      {/* ── Hero ── */}
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        className="text-center max-w-xl mx-auto mb-14">
        <p className="lamidone-gradient-text text-[10px] tracking-[0.4em] uppercase font-bold mb-4">Pricing</p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <div className="relative">
            <select
              ...
              aria-label="Currency"
              className="lamidone-card border rounded-xl px-4 py-2.5 pr-8 text-sm font-medium text-gray-700 dark:text-white/80 appearance-none cursor-pointer focus:outline-none focus:border-[#2563EB]/50 transition-colors"
            >
              {CURRENCIES.map(c => (
      ...
      <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-5 mb-20">
        {PLANS.map((plan, i) => (
          <motion.div key={plan.name}
            ...
            className={`relative flex flex-col rounded-2xl border p-7 lamidone-card
              ${plan.highlight
            ...
            <ul className="flex flex-col gap-2 border-t border-gray-100 dark:border-white/6 pt-5">
              {plan.features.map((f) => (
                <li key={f} className="flex items-start gap-2 text-xs text-gray-500 dark:text-white/50">
                  <span className="lamidone-gradient-text shrink-0 mt-0.5">✓</span>
                  {f}
                </li>
              ))}
            </ul>
        ...
        <div className="flex flex-col gap-3">
          {FAQ.map((item, i) => (
            <motion.div key={i}
              ...
              className={`lamidone-card border rounded-2xl overflow-hidden transition-all duration-200
            ...
            >
              <button type="button" onClick={() => setOpen(open === i ? null : i)}
                ...
                <span className={`text-sm font-semibold transition-colors ${open === i ? "lamidone-gradient-text" : "text-gray-800 dark:text-white"}`}>
                  {item.q}
                </span>
