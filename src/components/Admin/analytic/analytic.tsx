...
function Section({
  ...
  return (
    <section className="bg-[#010101]/80 border border-[#1f1f1f] rounded-lg p-4 shadow-md backdrop-blur-md">
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        setAnalytics(fallbackData); // fallback
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchActiveUsers = async () => {
      try {
        ...
        setActiveUsers({ active: 120 }); // fallback
      }
    };
    ...
    <div className="space-y-6">
      <Section title="🟢 Active Users">
        <MetricCard
          ...
          value={activeUsers.active.toString()}
        />
      </Section>

      <Section title="📊 Traffic Overview (Week over Week)">
        <Line data={trafficChart} options={{ responsive: true }} />
        <div className="mt-2 text-gray-300">
          Visits this week:{" "}
          {analytics.traffic.visits.reduce((a, b) => a + b, 0).toLocaleString()}
          <TrendIndicator
            current={analytics.traffic.visits.reduce((a, b) => a + b, 0)}
            prev={analytics.traffic.prevWeek?.reduce((a, b) => a + b, 0)}
          />
        </div>
      </Section>
      ...
      <Section title="📈 Top Pages">
        <ul className="space-y-2 text-sm text-gray-300">
          {analytics.topPages.map((page) => (
            <li
              key={page.url}
              className="flex justify-between items-center bg-[#0f0f0f]/60 px-4 py-2 rounded"
            >
              <span>{page.url}</span>
