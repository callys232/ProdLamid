          ...
          >
            ...
            <div className="flex items-center gap-3 px-6 py-5 border-b border-gray-100 dark:border-white/8 flex-shrink-0">
              ...
              <button
                type="button"
                onClick={onClose}
                className="flex h-8 w-8 items-center justify-center rounded-xl border border-gray-200 dark:border-white/10 text-gray-400 hover:text-gray-700 dark:hover:text-white hover:border-[#C12129]/30 transition-all"
                aria-label="Close"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <form onSubmit={onSearch} className="px-6 pt-5 pb-4 flex-shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/[0.04] text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-600 focus:outline-none focus:border-[#C12129]/50 focus:ring-1 focus:ring-[#C12129]/20 transition-colors"
                />
                {query && (
                  <button type="submit" className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-semibold text-[#C12129] hover:underline">
                    Search
                  </button>
                )}
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-3 [&::-webkit-scrollbar]:hidden">
              {loading && (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  <Loader2 className="w-6 h-6 text-[#C12129] animate-spin" />
              ...
              {!loading && experts.length === 0 && fetched && (
                <div className="flex flex-col items-center justify-center py-14 gap-4 text-center">
                  ...
                  <div className="flex gap-2">
                    <a href="/signup" className="px-4 py-2 rounded-xl text-xs font-semibold text-white bg-[#C12129] hover:bg-[#a01a20] transition-colors shadow-[0_0_14px_rgba(193,33,41,0.3)]">Create Account</a>
                    <a href="/jobs"   className="px-4 py-2 rounded-xl text-xs font-semibold border border-[#C12129]/25 text-[#C12129] hover:bg-[#C12129]/8 transition-colors">Browse Experts</a>
                  </div>
                </div>
              )}
              ...
              {!loading && experts.map((exp, i) => (
                <motion.div
                  ...
                  className="aivora-card border rounded-2xl p-4 flex items-start gap-3 hover:border-[#C12129]/25 transition-colors group"
                >
                  {/* Avatar */}
                  <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-[#C12129]/12 border border-[#C12129]/20">
                    {exp.avatar
                      ? <img src={exp.avatar} alt={exp.name} className="h-full w-full rounded-xl object-cover" />
                      : <span className="text-xs font-bold text-[#C12129]">{exp.name.slice(0, 2).toUpperCase()}</span>
                    }
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <BadgeCheck className="w-3.5 h-3.5 text-[#C12129] shrink-0" />
                    </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    <a href={`/jobs`} className="flex items-center gap-0.5 text-[9px] font-semibold text-[#C12129] hover:underline">
            ...
            {!loading && experts.length > 0 && (
              <div className="px-6 py-4 border-t border-gray-100 dark:border-white/8 flex items-center justify-between flex-shrink-0">
                ...
                <a href="/jobs" className="flex items-center gap-1 text-xs font-semibold text-[#C12129] hover:underline">
