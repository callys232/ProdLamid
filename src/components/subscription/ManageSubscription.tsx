...
export default function ManageSubscription() {
  ...
  return (
    <div className="space-y-5">
      ...
      {isFree && (
        <div className="rounded-xl border border-[#2563EB]/20 bg-[#2563EB]/5 p-5">
      ...
      {isActive && info?.tier === "premium" && (
        <div className="rounded-xl border border-blue-500/10 bg-blue-500/5 p-5">
          <h3 className="mb-2 text-sm font-semibold text-blue-400">Cancel Subscription</h3>
          ...
          {!confirm ? (
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
              ...
              className="rounded-lg border border-blue-500/30 px-4 py-2 text-xs font-semibold text-blue-400 transition hover:bg-blue-500/10">
              Cancel Subscription
            </motion.button>
          ) : (
            <div className="flex gap-3">
              <motion.button whileTap={{ scale: 0.97 }} onClick={handleCancel} disabled={cancelling}
                className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white transition hover:bg-blue-700 disabled:opacity-50">
      ...
      <a href="/pricing" className="block text-center text-xs text-gray-500 hover:text-[#2563EB] transition">
        View all plans →
      </a>
    </div>
  );
