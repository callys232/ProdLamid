      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mb-12">
        {(["monthly", "quarterly", "annual"] as BillingCycle[]).map(c => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={() => setCycle(c)}
            className={`relative rounded-full px-5 py-2 text-sm font-semibold transition ${cycle === c ? "bg-[#c12129] text-white" : "border border-white/10 text-gray-400 hover:text-white"
              }`}
          >
            {CYCLE_DISPLAY[c]}
            {c === "annual" && (
      ...
      <div className="mx-auto max-w-6xl grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {TIERS.map((t, i) => (
          <motion.div
            ...
            className={`relative flex flex-col rounded-2xl border p-6 transition-all ${t.highlight
            ...
            <ul className="mt-auto space-y-2.5">
              {t.features.map(f => (
                <li key={f} className="flex items-start gap-2.5 text-xs text-gray-400">
                  <CheckCircle className={`mt-0.5 h-3.5 w-3.5 flex-shrink-0 ${t.key === "enterprise_plus" ? "text-purple-400" : "text-[#c12129]"
                    }`} />
                  {f}
                </li>
              ))}
