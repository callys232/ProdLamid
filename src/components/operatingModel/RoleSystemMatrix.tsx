...
import type { ThronePosition } from "@/mocks/sovereign";

interface Props { thrones: ThronePosition[] }
...
export default function ThroneMatrix({ thrones }: Props) {
  return (
    <section className="px-6 py-12 border-t border-[#C9A84C]/10" style={{ background: "#0a0e1a" }}>
      <div className="max-w-6xl mx-auto">
        <motion.div {...g(0)} className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            <p className="text-[9px] tracking-[0.4em] uppercase text-[#C9A84C]/70 mb-1">Throne-to-System Mapping Matrix</p>
            <h2 className="text-xl font-bold text-[#E8E0CC]" style={{ fontFamily: "Georgia, serif" }}>Throne-to-System Mapping Matrix</h2>
            <p className="text-xs text-[#E8E0CC]/40 mt-1">Seven throne positions — each anchored to a designated enterprise system</p>
          </div>
          <div className="border border-[#C9A84C]/20 px-3 py-1.5 text-[9px] tracking-[0.2em] uppercase text-[#C9A84C]/70">
            Phase II — Sovereign Architecture
          </div>
        </motion.div>
          ...
          {thrones.map((t, i) => (
            <motion.div key={t.number} {...g(i * 0.07)}
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <p className="text-[9px] tracking-[0.25em] uppercase text-[#E8E0CC]/35">Throne Position</p>
              </div>
        ...
        <motion.p {...g(0.5)} className="mt-4 text-[10px] text-[#E8E0CC]/30 text-center tracking-widest">
          7 THRONE POSITIONS · 7 ENTERPRISE SYSTEMS BOUND · PHASE II OPERATIONAL
        </motion.p>
      </div>
    </section>
  );
